"""Instelling service — groepsinstellingen opslaan en ophalen."""
import logging
...
class InstellingService:
    ...
    def haal_alle(self, groep_id: int) -> dict[str, str]:
        ...
        rijen = self.db.query(AppInstelling).filter(AppInstelling.groep_id == groep_id).all()
    ...
    def haal_waarde(self, groep_id: int, sleutel: str) -> str:
        ...
        rij = self.db.query(AppInstelling).filter(
            AppInstelling.groep_id == groep_id,
            AppInstelling.sleutel == sleutel,
        ).first()
        return rij.waarde if rij else INSTELLING_SLEUTELS[sleutel]["standaard"]

    def sla_op(self, groep_id: int, sleutel: str, waarde: str, gebruiker_id: int) -> None:
        ...
        if rij:
            ...
            self.db.add(rij)
        self.db.commit()
        logger.info("Instelling %s = %s opgeslagen voor groep %d", sleutel, waarde, groep_id)
