...
def lijst(
    ...
    gebruiker_ids = {l.gebruiker_id for l in lidmaatschappen}
    gebruikers_map = {
        g.id: g for g in db.query(Gebruiker).filter(Gebruiker.id.in_(gebruiker_ids), Gebruiker.is_actief == True).all()
    } if gebruiker_ids else {}
    leden_per_team: dict[int, list] = {t.id: [] for t in teams}
    for l in lidmaatschappen:
        if l.gebruiker_id in gebruikers_map:
            leden_per_team[l.team_id].append(gebruikers_map[l.gebruiker_id])
...
def leden(
    ...
    alle_gebruikers = svc.haal_gebruikers_voor_locatie(actieve_locatie_id)
...
def voeg_lid_toe(
    ...
    try:
        svc.voeg_lid_toe(team.id, lid_gebruiker_id, lid_type)
...
def verwijder_lid(
    ...
    try:
        ...
        return RedirectResponse(url=f"/teams/{uuid}/leden?fout=fout.laatste_lidmaatschap", status_code=303)
    db.add(AuditLog(
